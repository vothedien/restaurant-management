...
def create_stocktake(data: StocktakeCreate, session: Db, actor: InventoryActor) -> dict:
...
@router.get("", dependencies=[Depends(require_inventory_permission("INVENTORY_MANAGE"))])
def list_stocktakes(
    session: Db,
...
@router.get(
    "/{stocktake_id}", dependencies=[Depends(require_inventory_permission("INVENTORY_MANAGE"))]
)
...
def complete_stocktake(
    stocktake_id: ResourceId, data: StocktakeComplete, session: Db, actor: InventoryActor
) -> dict:
    data = data.model_copy(update={"completed_by": actor.user_id})
